...
def get_dataset(set_name, subset_name):
    ...
    return load_dataset(set_name, subset_name, ignore_verifications=True,cache_dir='/gpfs1/scratch/ckchan666/xtreme')

def summary():
    for task in TASK:
        if 'train' in TASK[task]:
            set_name, subset_name, split = TASK[task]['train']    
            print('training data for ' + task +' : '+str(len(get_dataset(set_name, subset_name)[split])))
        if 'validation' in TASK[task]:
            if type(TASK[task]['validation']) is tuple:
                set_name, subset_name, split = TASK[task]['validation']    
                print('validation data for ' + task +' : '+str(len(get_dataset(set_name, subset_name)[split])))
            else:
                for split in TASK[task]['validation']:
                    set_name, subset_name, split = TASK[task]['validation'][split] 
                    print('validation data ('+split+') for ' + task +' : '+str(len(get_dataset(set_name, subset_name)[split])))
        if 'test' in TASK[task]:
            if type(TASK[task]['test']) is tuple:
                set_name, subset_name, split = TASK[task]['test']    
                print('test data for ' + task +' : '+str(len(get_dataset(set_name, subset_name)[split])))
            else:
                for lan in TASK[task]['test']:
                    set_name, subset_name, split = TASK[task]['test'][lan] 
                    print('test data ('+lan+') for ' + task +' : '+str(len(get_dataset(set_name, subset_name)[split])))
        if 'src' in TASK[task]:
            for lan in TASK[task]['src']:
                set_name, subset_name, split = TASK[task]['src'][lan] 
                print('training data ('+lan+') for ' + task +' : '+str(len(get_dataset(set_name, subset_name)[split])))