...
def train(
    ...
    panx_ds = xtreme_ds.panxTrainDataset()
    panx_ds_dataloader = torch.utils.data.DataLoader(
        panx_ds, batch_size=2, num_workers=0, shuffle=True
    )
    ...
    scheduler = transformers.get_linear_schedule_with_warmup(
        ...
        num_training_steps=len(panx_ds) // gradient_acc_size * task_config["epochs"],
    )
    log_step_size = len(panx_ds) // gradient_acc_size * task_config["epochs"] // 20
    import gc
    ...
    for _ in range(task_config["epochs"]):
        for batch in panx_ds_dataloader:
            ...
            #  model & input to cpu
            panxLoss = panxLoss + Output["loss"].item()
            finetune_model.taskmodels_dict[task].cpu()
            del Output
            batch.clear()
            del batch
            ...
            if (i + 1) % (gradient_acc_size / batch_size) == 0:
                ...
                if gradient_step_counter % log_step_size == 0:
                    ...
                    print(
                        ...
                        + "): "
                        + str(panxLoss / (log_step_size * gradient_acc_size))
                    )
                    print(
                        "disentangle loss ("
                        + str(gradient_step_counter)
                    ...
                    writer.add_scalar(
                        " loss",
                        panxLoss / (log_step_size * gradient_acc_size),
...
def test(finetune_model):
    test_dataloader = torch.utils.data.DataLoader(
        xtreme_ds.panxTestDataset(), batch_size=1, num_workers=0, shuffle=True
    )
