...
import torch
model.load_state_dict(torch.load('/gpfs1/home/ckchan666/mlm_disentangle_experiment/model/mlm/default/pytorch_model.bin'))
...
with torch.no_grad():
    for i_batch,batch in enumerate(dataloader):
        ...
        for i,s_id in enumerate(batch["source_id"]):
            ...
            bucc2018_source[lan][s_id] = torch.mean(bucc2018_source[lan][s_id], dim = 0)
        del source_output
        ...
        for i,s_id in enumerate(batch["target_id"]):
            ...
            bucc2018_target[lan][s_id] = torch.mean(bucc2018_target[lan][s_id], dim = 0)
import torch
...
with torch.no_grad():
    for i_batch,batch in enumerate(dataloader):
        ...
        for i,s_id in enumerate(batch["source_id"]):
            ...
            tatoeba_source[lan][s_id] = torch.mean(tatoeba_source[lan][s_id], dim = 0)
        del source_output
        target_output = disentangled_model(batch['target_tokens'])
        for i,s_id in enumerate(batch["target_id"]):
            ...
            tatoeba_target[lan][s_id] = torch.mean(tatoeba_target[lan][s_id], dim = 0)
import torch
...
with torch.no_grad():
    for i_batch,batch in enumerate(dataloader):
        ...
        for i,s_id in enumerate(batch["tokens"]):
            udpos_example[i_batch] = {'vectors':output.last_hidden_state[i],
...
import pickle
filehandler = open(
        "/gpfs1/scratch/ckchan666/pickle/udpos_example.pickle", "wb"
    )
